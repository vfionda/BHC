...
dataset_path="./dataset/ErdosRenyi"
...
for net in os.listdir(dataset_path):
    if (net=='.DS_Store'):
        continue
    print (net)
    g = nx.read_edgelist(os.path.join(dataset_path, net), nodetype=int)
    os.mkdir(os.path.join(res_path,"ErdosRenyi"))
#for g in graphs:
    for algo in detection_algorithms:
        with open('./experiments/ErdosRenyi/'+net+'-'+algo+'.txt', 'a') as f:
        #with open('./experiments/karate/'+algo+'.txt', 'a') as f:
            coms=computeCommunities(algo,g)
            ncom=len(coms.communities)
            print("number of communities "+str(ncom))
            for i in range(ncom):
                f.write("community "+str(i)+" "+str(coms.communities[i])+"\n")
            f.write("-------------------"+"\n")
            f.flush()
            for i in range(ncom):
                target_community=coms.communities[i]
